...
var versionedMigrations = []struct {
	...
	},
	{
		...
		stmts: []string{
			...
			`INSERT INTO transfers_new (id,run_id,remote_path,local_path,size_bytes,bytes_xferred,duration_ms,status,error_msg,started_at,finished_at) SELECT id,run_id,remote_path,local_path,size_bytes,bytes_xferred,duration_ms,status,error_msg,started_at,finished_at FROM transfers`,
	...
	},
	{
		...
		stmts: []string{
			...
			`INSERT INTO runs_new SELECT id,job_id,status,started_at,finished_at,total_files,copied_files,skipped_files,failed_files,total_size_bytes,0,NULL,error_msg FROM runs`,
			`DROP TABLE runs`,
			`ALTER TABLE runs_new RENAME TO runs`,
			`PRAGMA foreign_keys=ON`,
		},
